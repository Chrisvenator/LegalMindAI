...
def main():
    if len(sys.argv) < 2:
        logging.error(
            "Usage: python main.py <mode> \n mode: setup - fill chroma db with legal text files from resources directory, \n question - query the chroma db for legal information")
        sys.exit(1)

    # Set up logging
    logging.basicConfig(filename='log.txt', format='%(asctime)s %(levelname)-8s %(message)s', encoding='utf-8',
                        level=logging.INFO)
    ...
    match (sys.argv[1]):
        ...
        case "question":
            ...
            try:
                question = input()
                print(app.runQuestion(question))
            except:
                logging.error("An error occurred while processing the query")
                sys.exit(1)

        case _:
            logging.warning("Invalid mode, please choose either setup or question")
            sys.exit(1)
